...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeCharwrite = 32;
uint16_t SizeCharnotify = 32;
...
#define COPY_MYSERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_CHARWRITE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0xa2,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_CHARNOTIFY_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0xa1,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomCharwriteHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            ...
            /* Check Offset: Bit 15 indicates more data coming (fragmented), lower 15 bits are the offset */
            /* Only process when Offset == 0 (start of write) and no continuation flag */
            uint16_t offset_value = attribute_modified->Offset & 0x7FFF;
            uint16_t continuation_flag = attribute_modified->Offset & 0x8000;
            
            APP_DBG_MSG("Write event: Offset=%d, Continuation=%d, Length=%d\n", 
                       offset_value, continuation_flag ? 1 : 0, attribute_modified->Attr_Data_Length);
            
            /* Process only complete writes (offset 0, no continuation) */
            if (offset_value == 0 && continuation_flag == 0)
            {
              /* Trigger notification callback with the written data */
              Notification.Custom_Evt_Opcode = CUSTOM_STM_CHARWRITE_WRITE_EVT;
              Notification.DataTransfered.pPayload = (uint8_t*)attribute_modified->Attr_Data;
            ...
            {
              APP_DBG_MSG("Skipping write event (fragment or continuation)\n");
            }
...
void SVCCTL_InitCustomSvc(void)
{
  ...
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: myService \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARNOTIFY:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMyserviceHdle, CustomContext.CustomCharnotifyHdle, SizeCharnotify, pPayload);
